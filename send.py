...
while True:
	print("\033[0mეს თული შექმნილია რომ გააგზავნო მესიჯები ანონიმურად")
	break
type = 0
try:
	if sys.argv[1] == "track":
...
if type == 1:
	...
	while True:
		...
		cc = input("\tშეიყვანე ქვეყნის კოდი (+-ის გარეშე) : ")
		if '+' in cc:
		        tc = list(cc)
		        tc.remove('+')
		        cc = ''.join(tc)
		...
		if len(cc) >= 4 or len(cc) < 1:
		        print('\n\nარასწორი ქვეყნის კოდი...\n\t\tქვეყნის კოდები ძირითადად 1-დან 3 ციფრს შეიცავს...\n')
		        continue
		...
		if not numbe.isdigit():
		            print('\n\nტელეფონის ნომერი მხოლოდ ციფრებს უნდა შეიცავდეს...\n')
		            continue
		receiver = '+' + numbe
		text = input("შეიყვანე მესიჯი გასაგზავნად : ")
